...
struct _mem_node
{
    ...
    unsigned long  nbytes;     /* Number of bytes allocated.                     */
...
void        allocate_mem_node(void *addr, unsigned long nbytes,
...
void
allocate_mem_node(void *addr, unsigned long nbytes, char *filename, int lineno)
{
    mem_node *n;
    char *fn;
...
#if DEBUG == 1
    fprintf(stderr, "Allocating %lu bytes of memory at %p\n",
            nbytes, addr);
#endif
...
void *
checked_malloc_fn(unsigned long size, char *filename, int lineno)
{
    void *mem;
...
void *
checked_calloc_fn(unsigned long nmemb, unsigned long size, char *filename, int lineno)
{
    void *mem;
...
print_memory_leaks(void)
{
    ...
    {
        fprintf(stderr,
                "Memory leak: %lu bytes allocated at %p in "
                "file: %s, line: %d.\n",
                n->nbytes, n->addr, n->filename, n->lineno);
    }
